...
rf_grid <- get_rf_grid(split_list, 4)
...
```{r}
## Random Forest Model
rf_grid <- get_rf_grid(split_list, num_models)
...
# build the gbm model
gbm_grid <- get_gbm_grid(split_list,  num_models)
...
get_glm <- function(split_list){
  ...
  h2o.rm(glm_model_name)
  
  # Train the GLM Model
  glm_fit <- h2o.glm(x = split_list$x, 
                     y = split_list$y, 
                     training_frame = split_list$train,
                     model_id = glm_model_name,
                     validation_frame = split_list$valid,
                     family = "binomial",
                     lambda_search = TRUE)
...
get_nb <- function(split_list){
  ...
  h2o.rm(nb_model_name)
  
  # Train the NB model
  nb_fit <- h2o.naiveBayes(x = split_list$x,
                           y = split_list$y,
                           training_frame = split_list$train,
                           model_id = nb_model_name)
