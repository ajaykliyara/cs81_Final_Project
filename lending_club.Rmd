...
```{r}
#### build the models
# first run the loan origination model
orig_file_name <- "data/loanstats_2015_optimized.csv"
orig_factor_cols <- c("is_bad", "home_ownership", "zip_code", "addr_state", "grade", "sub_grade", "purpose")
...
combined <- c(orig_col, note_trading_col)
#df <- df %>% select(one_of(combined))
df <- df %>% select(one_of(orig_col))

# columns to delete
col_to_delete <- c("title", "collections_12_mths_ex_med", "mths_since_last_major_derog")
...
split_list <- get_split_list(data, factor_cols)
rf_grid <- get_rf_grid(split_list, 2)
