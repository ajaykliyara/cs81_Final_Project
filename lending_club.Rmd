...
# select the best 12 predictors (home ownership no longer included)
...
data <- as.h2o(df1)
rf_model <- get_rf_grid(data, factor_cols)
...
# set up the factor list indicating what columns are factors in df1
factor_cols <- c("is_bad", "zip_code", "addr_state")
```
...
# build the gbm model
gbm_model <- get_gbm_grid(data, factor_cols, 50)
```
...
get_dl_grid <- function(data, factor_cols, in_max_run_time_sec = 0, 
  ...
  activation_opt <- c("Rectifier", "RectifierWithDropout", "Maxout", "MaxoutWithDropout", "Tanh")
  l1_opt <- c(0, 0.00001, 0.0001, 0.001, 0.01, 0.1)
  l2_opt <- c(0, 0.00001, 0.0001, 0.001, 0.01, 0.1)
  hyper_params <- list(activation = activation_opt,
                       l1 = l1_opt,
                       l2 = l2_opt)
  search_criteria <- list(strategy = "RandomDiscrete", max_runtime_secs = in_max_run_time_sec)
...
dl_model <- get_dl_grid(data, factor_cols)
```
