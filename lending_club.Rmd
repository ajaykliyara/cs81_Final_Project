...
get_dl_grid <- function(data, factor_cols, in_max_run_time_sec = 0, 
                        ...
                        in_grid_id = dl_grid_name, in_max_runtime_secs = 0){
...
dl_grid <- get_dl_grid(data, factor_cols, 
                       ...
                       max_epochs = 1000)
# dl_model <- get_best_model(split_list, dl_grid_name)
```
...
```{r}
### ensemble stacking
get_h2o_ensemble <- function(model_list){
...
model_list <- c(rf_models, gbm_models, glm_model, nb_model)

ensemble_perf <- get_h2o_ensemble(model_list)
print(ensemble_perf)
```

```{r}
#### build the loan marketplace models
marketplace_file_name <- "data/loanstats_2015_marketplace_optimized.csv"
market_factor_cols <- c("is_bad", "zip_code")
...
auc_list$random_forest <- h2o.auc(mp_perf_rf)
...
# build the gbm model
mp_gbm_grid <- get_gbm_grid(split_list,  num_models, nfolds)
mp_gbm_model <- get_best_model(mp_gbm_grid)
mp_perf_gbm <- get_test_perf(split_list, mp_gbm_model, "GBM")
auc_list$mp_gbm <- h2o.auc(mp_perf_gbm)
```
...
if (loan_marketplace == TRUE){
  ...
  best_predictors <- best_predictors[1:6]
}else{
