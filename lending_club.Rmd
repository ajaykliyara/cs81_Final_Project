...
get_rf_grid <- function(data, factor_cols, max_num_models = 25, 
  ...
  best_model <- get_best_model(split_list, rf_grid_id)
...
best_model <- get_rf_grid(data, factor_cols, 100)
...
get_gbm_grid <- function(data, factor_cols, max_num_models = 25, 
  ...
  gbm_gridperf <- h2o.getGrid(grid_id = in_grid_id, 
                              sort_by = "auc", 
                              decreasing = TRUE)
  print(gbm_gridperf)
  ...
  # make a prediction using the best model and the test data
  gbm_perf <- h2o.performance(model = best_model,
                              newdata = test)
  
  cat("Best GBM Model AUC Computed on Independent Test Data: ", h2o.auc(gbm_perf))
